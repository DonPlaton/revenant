...
launcher script, or the sessions themselves, as tabs of one terminal window or
one window each.
...
def render_launcher(
    ...
    *,
    ...
    """Build a script that opens every session, as tabs of one window or one window each."""
    ...
    if shell == "cmd":
        # `start` is the only thing a .cmd file has, and it opens a console window.
        # Saying so beats a launcher that quietly ignores --layout tabs.
        head = [
            "@echo off",
            "rem generated by Revenant",
            "rem one console window per session; use a .ps1 launcher for tabs",
            "",
        ]
        body = [
    ...
    if shell == "bash":
        chosen = terminal or terminals.choose(layout=layout)
        head = [
            "#!/usr/bin/env bash",
        ...
        plan = chosen.plan(jobs, layout=layout, window=window, profile=profile)
        body = [plan.render()]
        if plan.note:
    ...
    if terminal is not None and not isinstance(terminal, terminals.WindowsTerminal):
        ...
        plan = terminal.plan(jobs, layout=layout, window=window, profile=profile)
        body = _render_ps_plan(plan)
        if plan.note:
            body.append(f"Write-Host {_quote_ps(plan.note)}")
        return "\n".join(head + skipped + ([""] if skipped else []) + body) + "\n"

    if layout == terminals.LAYOUT_WINDOWS:
        # A window per session needs no wt.exe at all. Start-Process opens a console
        # each, and on Windows 11 that console is Windows Terminal anyway whenever
        # it is the default terminal application, so the look is not lost.
        head = [
            "# generated by Revenant",
            "# Opens every session in a window of its own.",
            "",
            "$ErrorActionPreference = 'Stop'",
    ...
    # Resolved where the script runs, not where it was written. The bare wt.exe in
    # WindowsApps is an app-execution alias that fails with ERROR_CANT_ACCESS_FILE
    # on machines where the alias is off, and the package's own copy does not.
    packages = ",\n".join(
        f"  \"$env:LOCALAPPDATA\\Microsoft\\WindowsApps\\{name}\\wt.exe\""
        for name in terminals.WT_PACKAGES
    )
    prologue = [
        "$shell = if (Get-Command pwsh -ErrorAction SilentlyContinue) { 'pwsh' } else { 'powershell' }",
        "$wt = @(",
...
def launch(
    ...
    *,
    ...
    # What the plan really did, not what was asked for: a backend that could not
...
def build_parser() -> argparse.ArgumentParser:
    ...
    output.add_argument(
        "--layout",
        choices=list(terminals.LAYOUTS),
        default=terminals.DEFAULT_LAYOUT,
        help="tabs of one window (default), or a window per session",
    )
...
def _print_terminals(stream) -> int:
    ...
    for cls in terminals.ALL:
        ...
        layouts = ", ".join(terminal.layouts)
        print(f"{terminal.key:<16} {terminal.label:<18} {state:<14} {layouts:<15}{chosen}", file=stream)
    return 0
...
def main(argv: Sequence[str] | None = None, *, stream=None) -> int:
    ...
    if args.no_tabs:
        # What --no-tabs meant in 1.0, said the way 1.3 says it.
        args.layout = terminals.LAYOUT_WINDOWS
        print("revenant: --no-tabs is deprecated; use --layout windows.", file=sys.stderr)
    ...
    if args.emit:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        chosen = (
            terminals.choose(args.terminal, layout=args.layout)
            if args.terminal or emit_shell == "bash"
            else None
        )
